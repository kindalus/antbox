...
import type { FeaturesEngine } from "../features/features_engine.ts";
import type { WorkflowsService } from "./workflows_service.ts";
import type { WorkflowInstancesService } from "./workflow_instances_service.ts";
...
export interface WorkflowInstancesEngineContext {
...
/**
 * WorkflowInstancesEngine - Handles workflow execution logic
 *
 * This engine is responsible for:
 * - Starting workflow instances
 * - Executing transitions between states
 * - Cancelling workflow instances
 * - Updating nodes within workflow context
 * - Executing workflow actions (onEnter, onExit, transition actions)
 */
export class WorkflowInstancesEngine {
	readonly #configRepo: ConfigurationRepository;
	readonly #nodeService: NodeService;
	readonly #workflowsService: WorkflowsService;
	readonly #workflowInstancesService: WorkflowInstancesService;
	readonly #featuresEngine: FeaturesEngine;

	constructor(ctx: WorkflowInstancesEngineContext) {
		this.#configRepo = ctx.configRepo;
		this.#nodeService = ctx.nodeService;
		this.#workflowsService = ctx.workflowsService;
		this.#workflowInstancesService = ctx.workflowInstancesService;
		this.#featuresEngine = ctx.featuresEngine;
	}
	...
	async startWorkflow(
		...
		const existingInstanceOrErr = await this.#workflowInstancesService
			.getWorkflowInstanceByNodeUuid(authCtx, nodeUuid);
	...
	async findActiveInstances(
		...
		const instancesOrErr = await this.#workflowInstancesService.listWorkflowInstances(authCtx);
	...
	async #executeActions(
		...
		for (const actionString of actions) {
			...
			// Execute the action using FeaturesEngine
			const result = await this.#featuresEngine.runAction(
				authCtx,
				actionUuid,
				[nodeUuid],
				params,
