...
export class CompositeSpecification<T> implements Specification<T> {
	constructor(
		readonly isSatisfiedBy: (c: T) => ValidationResult,
	) {}
...
export class AndSpecification<T> extends CompositeSpecification<T> {
	constructor(s1: Specification<T>, s2: Specification<T>, ...s: Specification<T>[]) {
		super(
			(candidate: T) => {
				const errors = [s1, s2, ...s].map((spec) => spec.isSatisfiedBy(candidate))
					.filter((r) => r.isLeft())
					.map((r) => r.value)
					.reduce(
...
export class OrSpecification<T> extends CompositeSpecification<T> {
	constructor(...specs: Specification<T>[]) {
		super(
			(candidate: T) => {
				const errors = specs.map((spec) => spec.isSatisfiedBy(candidate))
					.filter((r) => r.isLeft())
					.map((r) => r.value)
					.reduce(
						(acc, err) => [...acc, ...(err as ValidationError).errors],
...
export function specFn<T>(fn: (candidate: T) => ValidationResult): Specification<T> {
