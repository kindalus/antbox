...
export type { NodeServiceContext } from "/application/node_service_context.ts";
...
export async function startServer(opts?: ServerOpts) {
	...
	const service = new AntboxService({
		...
		repository: await providerFrom(opts?.repository) ?? new InMemoryNodeRepository(),
		storage: await providerFrom(opts?.storage) ?? new InMemoryStorageProvider(),
	});
...
export async function printKeys(opts?: { passwd?: string; symmetricKey?: string; jwkPath?: string }) {
	console.log("Root passwd:\t", opts?.passwd ?? ROOT_PASSWD);

	console.log("Symmetric Key:\t", opts?.symmetricKey ?? SYMMETRIC_KEY);
	console.log("JSON Web Key:\t", JSON.stringify(await loadJwk(opts?.jwkPath), null, 4));
}
...
async function providerFrom<T>(cfg?: ModuleConfiguration): Promise<T | undefined> {
	if (!cfg) {
		return;
	}
...
function loadModule<T>(modulePath: string): Promise<(...p: string[]) => Promise<Either<AntboxError, T>>> {
