...
**Naming Convention:** Use **kebab-case** for `uuid` (e.g., `invoice-data`, `project-tracker`). The `name` property within the Aspect's own definition will also typically be kebab-case for its own identifier.

**Best Practice: Aspect Composability**
Aspects are designed to be small, orthogonal, and reusable building blocks. They are *composable*, meaning multiple aspects can be applied to a single node. To maintain this composability and avoid creating "monolithic" aspects, it is recommended to keep the number of `properties` in an `AspectNode` **no greater than 5**. If more properties are needed, consider splitting them into multiple, more focused aspects.

```typescript
interface AspectNode extends Node {
  name: string; // Internal identifier for the aspect type (e.g., "invoice", "legal-review")
  properties: AspectProperty[];
}

interface AspectProperty {
  name: string;        // Property key (e.g., "totalAmount")
  ...
  validationList?: string[]; // Enum values for string types
}
```
...
**Examples**:
*   *File is a PDF*: `['mimetype', '==', 'application/pdf']`
*   *Has 'Invoice' Aspect*: `['aspects', 'contains', 'invoice-data']`
*   *Amount > 1000*: `['invoice-data.amount', '>', 1000]`
...
export default {
  ...
  parameters: [
    { name: "uuids", type: "array", arrayType: "string", required: true, description: "List of contract Node UUIDs" }
  ],
...
export default {
  ...
  parameters: [
    { name: "uuid", type: "string", required: true, description: "The UUID of the node" }
  ],
...
export default {
  ...
  parameters: [
    { name: "uuids", type: "array", arrayType: "string", required: true, description: "UUIDs of created PDFs" }
  ],
  ...
  run: async (ctx, args) => {
    for (const uuid of args.uuids) {
      ...
      const node = nodeResult.value;
      console.log(`[AUTOMATED ACTION] New PDF created: "${node.title}" (UUID: ${uuid})`);
    }
  }
};
```