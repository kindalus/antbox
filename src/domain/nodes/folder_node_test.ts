...
describe("FolderNode", () => {
	describe("create", () => {
		...
		it("should always have application/vnd.antbox.folder mimetype", () => {
			const result = FolderNode.create({
				title: "Title",
		...
		it("should always have valid values for 'anonymous', 'group', 'authenticated', 'advanced' in permissions", () => {
			const metadatas = [
				{
					title: "Title",
					owner: "user@domain.com",
			...
			metadatas.forEach((metadata) => {
				const result = FolderNode.create(metadata as unknown as NodeMetadata);
