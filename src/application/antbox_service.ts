...
import { GroupUpdatedEvent } from "../domain/auth/group_updated_event .ts";
...
export class AntboxService {
	...
	constructor(nodeCtx: NodeServiceContext, ocrEngine: OcrEngine) {
		...
		this.apiKeysService = new ApiKeyService(
			this.nodeService,
			nodeCtx.uuidGenerator,
		);
	...
	async createFile(
		...
		const parent = metadata.parent ?? Folders.ROOT_FOLDER_UUID;
		if (FolderNode.isSystemFolder(parent)) {
			return left(
				new BadRequestError("Cannot create regular files in system folder"),
			);
		}
		...
		if (
			!filtersSpecFrom(parentOrErr.value.childFilters).isSatisfiedBy(
				metadata as Node,
			)
		) {
			return left(
				new BadRequestError("Node does not satisfy parent folder filters"),
			);
		}
	...
	async create(
		...
		if (FolderNode.isRootFolder(metadata.parent!) && !Node.isFolder(metadata)) {
			return left(
				new BadRequestError("Cannot create regular files in root folder"),
			);
		}

		const parentOrErr = await this.#getFolderWithPermission(
			authCtx,
			metadata.parent ?? Folders.ROOT_FOLDER_UUID,
			"Write",
		);
	...
	async #getFolderWithPermission(
		...
		const voidOrErr = this.#assertPermission(
			auth,
			folderOrErr.value,
			permission,
		);
	...
	#assertPermission(
		...
		const hasAdvancedPermission = Object.entries(
			node.permissions.advanced ?? {},
		)
	...
	async get(
		...
		if (FolderNode.isSystemFolder(uuid)) {
			return right(
				FolderNode.SYSTEM_FOLDERS.find((folder) => folder.uuid === uuid)!,
			);
		}
	...
	async find(
		...
		const result = await this.nodeService.find(
			noSystemNodes,
		...
		const forbidenNodes = parentNodes.value.nodes
			.filter((n) => {
		...
		const filteredNodes = result.value.nodes.filter(
			(n) => !forbidenNodes.includes(n.parent),
		);

		const sliced = filteredNodes.slice(
			(pageToken - 1) * pageSize,
			pageToken * pageSize,
		);
	...
	#removeSystemNodesUnlessRequested(filters: NodeFilter[]): NodeFilter[] {
		...
		const withMimetype = filters.filter(
			([field, _operator, _value]) => field === "mimetype",
		);
		...
		withMimetype.forEach(([_, operator, value]) => {
			...
			if (operator === "in") {
				systemNodes = systemNodes.filter(
	...
	async update(
		...
		if (
			!filtersSpecFrom(parentOrErr.value.childFilters).isSatisfiedBy(candidate)
		) {
			return left(
				new BadRequestError("Node does not satisfy parent folder filters"),
			);
		...
		if (node.isFolder()) {
			return this.updateFolder(authCtx, node, diffs as Partial<FolderNode>);
		}
	...
	async delete(
		...
		if (Nodes.isUser(nodeOrErr.value)) {
		...
		if (
			nodeOrErr.value.isFolder() &&
			this.#assertCanWrite(authCtx, nodeOrErr.value).isLeft()
		) {
			return left(new ForbiddenError());
		}
		...
		if (voidOrErr.isRight()) {
			DomainEvents.notify(
				new NodeDeletedEvent(authCtx.principal.email!, nodeOrErr.value),
			);
		}
	...
	async runAction(
		...
		const myGroups = [
			authCtx.principal.group ?? Anonymous.uuid!,
			...authCtx.principal.groups,
		];
	...
	createOrReplaceAspect(
		authCtx: AuthContextProvider,
		aspect: Partial<AspectNode>,
	) {
		if (!authCtx.principal.isAdmin()) {
			return Promise.resolve(left(new ForbiddenError()));
		}
	...
	getGroup(
		authCtx: AuthContextProvider,
		uuid: string,
	): Promise<Either<AntboxError, Group>> {
		if (!authCtx.principal.isAdmin()) {
			return Promise.resolve(left(new ForbiddenError()));
		}
	...
	async recognizeText(
		...
		if (!recognizableMimetypes.includes(nodeOrErr.value.mimetype)) {
			return left(
				new BadRequestError(
					`File type not supported: ${nodeOrErr.value.mimetype}`,
				),
			);
		}
