...
describe("AspectNode", () => {
	describe("create", () => {
		...
		it("should initialize with properties and filters", () => {
		...
		it("should always have aspect mimetype", () => {
			const result = AspectNode.create({
				title: "Test aspect",
		...
		it("should always have aspects folder as parent", () => {
			const result = AspectNode.create({
				title: "Test aspect",
		...
		it("should initialize with various property types", () => {
			const properties: AspectProperty[] = [
				{
					name: "string_prop",
	...
	describe("update", () => {
		it("should modify title, fid, description and filters", async () => {
			const createResult = AspectNode.create({
	...
	describe("validation", () => {
		it("properties should support validation configuration", () => {
			const properties: AspectProperty[] = [
				{
			...
			const validatedString = aspectNode.properties.find((p) => p.name === "validated_string");
			expect(validatedString?.validationRegex).toBe("^[A-Z]+$");
			expect(validatedString?.searchable).toBe(true);

			const listProperty = aspectNode.properties.find((p) => p.name === "list_property");
			...
			const filteredProperty = aspectNode.properties.find((p) => p.name === "filtered_property");
			...
			const readonlyProperty = aspectNode.properties.find((p) => p.name === "readonly_property");
			expect(readonlyProperty?.readonly).toBe(true);
			expect(readonlyProperty?.default).toBe("readonly_value");
		});

		it("should handle empty filters and properties gracefully", () => {
			const result = AspectNode.create({
				title: "Empty aspect",
				owner: "user@domain.com",
				properties: [],
