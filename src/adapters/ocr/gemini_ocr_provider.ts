 ...
 *
 * @example
 * const provider = new GeminiOCRProvider("gemini-2.5-flash");
 */
export class GeminiOCRProvider implements OCRProvider {
	readonly #client: GoogleGenAI;
	readonly #modelName: string;

	constructor(modelName = "gemini-2.5-flash", apiKey?: string) {
	...
	async ocr(file: File): Promise<Either<AntboxError, string>> {
		try {
			...
			const response = await this.#client.models.generateContent({
				...
				contents: [{
					...
					parts: [
						{
							text:
								"Extract all text content from this document or image. Return only the extracted text, preserving structure where possible.",
						},
						{
							inlineData: {
								mimeType: file.type,
			...
			return left(
				new AntboxError("GeminiOCRError", `OCR failed for file ${file.name}: ${error}`),
			);
		}
	}
...
export default function buildGeminiOCRProvider(
	...
	return Promise.resolve(
		right(new GeminiOCRProvider(modelName ?? "gemini-2.5-flash", apiKey)),
	);
}
