...
import { Collection, Db, Document, Filter, MongoClient, ObjectId, WithId } from "/deps/mongodb";
...
import MurmurHash3 from "/deps/murmurhash";
...
export default function buildMongodbNodeRepository(
	...
	return new MongoClient(url).connect()
		...
		.catch((err) => left(new UnknownError(err.message))) as Promise<Either<AntboxError, NodeRepository>>;
}
...
export class MongodbNodeRepository implements NodeRepository {
	...
	constructor(client: MongoClient, dbname: string) {
		...
		this.#collection = this.#db.collection(MongodbNodeRepository.#COLLECTION_NAME);
	}
	...
	async filter(
		...
		const [count, findCursor] = await Promise.all([
			...
			this.#collection.find(query, { limit: pageSize, skip: pageSize * (pageToken - 1) }),
		]);
	...
	async update(node: Node): Promise<Either<NodeNotFoundError, void>> {
		...
		try {
			await this.#collection.updateOne(toObjectId(node.uuid), { $set: { ...node } });
		} catch (err) {
			return left(new MongodbError(err.message));
		}
