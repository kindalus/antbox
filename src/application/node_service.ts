...
import { Node, FileNode } from "../domain/nodes/node.ts";
...
export class NodeService {
  ...
  constructor(private readonly context: NodeServiceContext) {
    ...
    this.#systemListCreator[Node.ACTIONS_FOLDER_UUID] =
      builtinActions.map(actionToNode);
    this.#systemListCreator[Node.ASPECTS_FOLDER_UUID] =
      builtinAspects.map(aspectToNode);
    this.#systemListCreator[Node.USERS_FOLDER_UUID] =
      builtinUsers.map(userToNode);
    this.#systemListCreator[Node.GROUPS_FOLDER_UUID] =
      builtinGroups.map(groupToNode);
  }
