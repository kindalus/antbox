...
Deno.test("propertyTypeSpec", () => {
	...
	assertEquals((r3.value as ValidationError).has(PropertyTypeError.ERROR_CODE), true);
});
...
Deno.test("requiredSpec", () => {
	...
	assertEquals((r0.value as ValidationError).has(RequiredPropertyError.ERROR_CODE), true);

	assertFalse(r1.isRight());
	assertEquals((r1.value as ValidationError).has(RequiredPropertyError.ERROR_CODE), true);

	assertEquals(r2.isRight(), true);
	assertFalse((r3.value as ValidationError).has(RequiredPropertyError.ERROR_CODE));

	assertFalse(r4.isRight());
	assertEquals((r4.value as ValidationError).has(RequiredPropertyError.ERROR_CODE), true);
});
